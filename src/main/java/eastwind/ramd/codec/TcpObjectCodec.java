...
import eastwind.ramd.support.RamdMagic;
...
public class TcpObjectCodec extends ByteToMessageCodec<TcpObject> {
	...
	protected void encode(ChannelHandlerContext ctx, TcpObject msg, ByteBuf out) throws Exception {
		...
		line.writeBytes(RamdMagic.MAGIC);
		line.writeShort(0);
		...
		line.setShort(RamdMagic.MAGIC.length, line.readableBytes() - RamdMagic.MAGIC.length + out.readableBytes());
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		if (in.readableBytes() < RamdMagic.MAGIC.length + 2) {
			return;
		}
		in.markReaderIndex();
		in.skipBytes(RamdMagic.MAGIC.length);
